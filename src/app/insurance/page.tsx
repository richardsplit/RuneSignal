...
export default function InsuranceDashboard() {
  ...
  const [claims, setClaims] = useState<any[]>(DEFAULT_CLAIMS);

  // Load from local storage
  useEffect(() => {
    const savedProfiles = localStorage.getItem('trustlayer_insurance_profiles');
    if (savedProfiles) setProfiles(JSON.parse(savedProfiles));

    const savedClaims = localStorage.getItem('trustlayer_insurance_claims');
    if (savedClaims) setClaims(JSON.parse(savedClaims));
  }, []);

  // Save to local storage
  useEffect(() => {
    localStorage.setItem('trustlayer_insurance_profiles', JSON.stringify(profiles));
    localStorage.setItem('trustlayer_insurance_claims', JSON.stringify(claims));
  }, [profiles, claims]);
  ...
  return (
    <>
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem', marginBottom: '2.5rem' }}>
          ...
          <div className="glass-panel" style={{ padding: '1.25rem' }}>
            ...
            <p style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--color-accent-amber)' }}>{claims.length}</p>
          </div>
        </div>
            ...
            <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-glass)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button 
                ...
                onClick={() => {
                  ...
                  setProfiles(prev => prev.map(p => ({ ...p, score: Math.max(0, p.score - Math.floor(Math.random() * 5)) })));
                }}
              >
                Recalculate All
              </button>
      ...
      <FileClaimModal 
        ...
        onSuccess={(claim: any) => setClaims(prev => [claim, ...prev])}
      />
    </>
  );
}
