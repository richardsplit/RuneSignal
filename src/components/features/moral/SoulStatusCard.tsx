        ...
        <div>
          ...
          {version ? (
            <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
              <div>
                <span style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem' }}>Version</span>
                <p style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--success)' }}>v{version}</p>
              </div>
              <div>
                <span style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem' }}>Signed By</span>
                <p style={{ fontSize: '0.95rem', fontWeight: 500 }}>{signedBy}</p>
              </div>
              <div>
                <span style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem' }}>Last Updated</span>
                <p style={{ fontSize: '0.95rem', fontWeight: 500 }}>{new Date(createdAt).toLocaleDateString()}</p>
              </div>
              <div>
                <span style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem' }}>Signature</span>
                <p style={{ fontSize: '0.95rem', fontWeight: 600, color: signatureValid ? 'var(--success)' : 'var(--danger)' }}>
                  {signatureValid ? '✓ Valid' : '✗ Tampered'}
                </p>
              </div>
            </div>
          ) : (
            <p style={{ color: 'var(--warning)' }}>No Corporate SOUL configured. Set one up to enable moral governance.</p>
          )}
        </div>
