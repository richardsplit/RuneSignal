...
/* ─── Maps ─────────────────────────────────────────────────────────────── */
export const STATUS_MAP: Record<ControlStatus, { label: string; color: string }> = {
...
export function StatusBadge({ status }: { status: ControlStatus }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.125rem 0.5rem', borderRadius: 999, fontSize: '0.6875rem', fontWeight: 600, background: `${color}1a`, color, border: `1px solid ${color}33` }}>
...
export function ControlCard({ control, evaluating, onEvaluate, onOpenIncident }: CardProps) {
  ...
  return (
    <div style={{ padding: '1rem 1.125rem', borderRadius: 'var(--radius-md)', background: 'var(--bg-surface-2)', border: `1px solid ${isFailing ? '#ef444430' : 'var(--border)'}`, display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '0.5rem' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {control.description && (
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', lineHeight: 1.4 }}>{control.description}</div>
          )}
        </div>
      ...
      {isFailing && <div style={{ height: 2, background: 'linear-gradient(90deg, #ef444480, transparent)', borderRadius: 1, marginTop: '0.25rem' }} />}
    </div>
  );
}
