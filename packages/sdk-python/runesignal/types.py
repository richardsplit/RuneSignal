"""RuneSignal Python SDK — Core Type Definitions"""
...
class RuneSignalError(Exception):
    def __init__(self, message: str, status: int = 0, code: Optional[str] = None):
        super().__init__(message)
        self.status = status
        self.code = code

class AuthenticationError(RuneSignalError):
    def __init__(self, message: str = "Invalid API key"):
        super().__init__(message, 401, "AUTHENTICATION_ERROR")

class RateLimitError(RuneSignalError):
    def __init__(self, message: str = "Rate limit exceeded"):
        super().__init__(message, 429, "RATE_LIMIT_ERROR")

class FirewallBlockError(RuneSignalError):
    def __init__(self, evaluation: "EvaluateResponse"):
        super().__init__(
            f"Action blocked: {'; '.join(evaluation.reasons)}", 403, "FIREWALL_BLOCK"
        )
